### START FUNCTION
"""This module contain the defination of a class named FieldDataProcessor that can used to create a DataFrame from a database, clean the dataframe by renaming the columns and applying a method to correct some of it data error and also to read data from a csv file

methods:
initialize_logging: Decribe the logger for the class

ingest_sql_data: return a dataframe from a database

rename_columns: rename some of the columns of the df generated by the ingest_sql_data method

apply_correction: apply correction to one of the columns data in the df to make the data consistent



process: This method is used to arrange the previous method to create a sequence of steps to automate the retrieval of data from a database and from a csv file. The method simulates an ETL process
"""
import pandas as pd
...
class FieldDataProcessor():
    ...
    def ingest_sql_data(self):
        """This method using the create_db_engine from the data_ingestion moduel to create dataframe populated with data from the database
        located in the db file.
        output:
